...
public class MenuSheetBehaviour<V extends View> extends CoordinatorLayout.Behavior<V> {
  ...
  public MenuSheetBehaviour(@NonNull Context context, @Nullable AttributeSet attrs) {
    super(context, attrs);
    ...
    if (a.hasValue(R.styleable.BottomSheetBehavior_Layout_shapeAppearance)) {
      this.shapeAppearanceModelDefault =
        ShapeAppearanceModel.builder(
            context, attrs, R.attr.bottomSheetStyle, DEF_STYLE_RES)
          .build();
    }
    ...
    this.elevation =
      a.getDimension(R.styleable.BottomSheetBehavior_Layout_android_elevation, -1);
    ...
    setGestureInsetBottomIgnored(
      a.getBoolean(
        R.styleable.BottomSheetBehavior_Layout_gestureInsetBottomIgnored, false));
    setFitToContents(
    ...
    setSaveFlags(
      a.getInt(R.styleable.BottomSheetBehavior_Layout_behavior_saveFlags, SAVE_NONE));
    setHalfExpandedRatio(
      a.getFloat(
        R.styleable.BottomSheetBehavior_Layout_behavior_halfExpandedRatio, 0.5f));
    ...
    paddingBottomSystemWindowInsets =
      a.getBoolean(
        R.styleable.BottomSheetBehavior_Layout_paddingBottomSystemWindowInsets,
        false);
    paddingLeftSystemWindowInsets =
      a.getBoolean(
        R.styleable.BottomSheetBehavior_Layout_paddingLeftSystemWindowInsets,
        false);
    paddingRightSystemWindowInsets =
      a.getBoolean(
        R.styleable.BottomSheetBehavior_Layout_paddingRightSystemWindowInsets,
        false);
    ...
    paddingTopSystemWindowInsets =
      a.getBoolean(
        R.styleable.BottomSheetBehavior_Layout_paddingTopSystemWindowInsets, true);
    marginLeftSystemWindowInsets =
      a.getBoolean(
        R.styleable.BottomSheetBehavior_Layout_marginLeftSystemWindowInsets, false);
    marginRightSystemWindowInsets =
      a.getBoolean(
        R.styleable.BottomSheetBehavior_Layout_marginRightSystemWindowInsets,
        false);
    marginTopSystemWindowInsets =
      a.getBoolean(
        R.styleable.BottomSheetBehavior_Layout_marginTopSystemWindowInsets, false);
    shouldRemoveExpandedCorners =
      a.getBoolean(
        R.styleable.BottomSheetBehavior_Layout_shouldRemoveExpandedCorners, true);
  ...
  public void onAttachedToLayoutParams(@NonNull LayoutParams layoutParams) {
    ...
    // These may already be null, but just be safe, explicitly assign them. This lets us know
    // the
  ...
  public boolean onLayoutChild(
    ...
    if (viewRef == null) {
      ...
      // Only set MaterialShapeDrawable as background if shapeTheming is enabled, otherwise
      // will
      // default to android:background declared in styles or layout.
      if (materialShapeDrawable != null) {
        ViewCompat.setBackground(child, materialShapeDrawable);
        // Use elevation attr if set on bottom/sheet; otherwise, use elevation of child
        // view.
    ...
    if (parentHeight - childHeight < insetTop) {
      if (paddingTopSystemWindowInsets) {
        // If the bottomsheet would land in the middle of the status bar when fully expanded
        // add
   ...
   * or if it is expanded in two stages (half the height of the parent container, full height
   * of parent container).
   */
   ...
   * if it is expanded in two stages (half the height of the parent container, full height of
   * parent container). Default value is true.
   *
  ...
  public void setFitToContents(boolean fitToContents) {
    ...
    // Fix incorrect expanded settings depending on whether or not we are fitting sheet to
    // contents.
    setStateInternal(
  ...
  /**
   * Sets the height of the bottom sheet when it is collapsed while optionally animating between
   * the old height and the new height.
   *
   * @param peekHeight The height of the collapsed bottom sheet in pixels, or {@link
   *                   #PEEK_HEIGHT_AUTO} to configure the sheet to peek automatically at 16:9 ratio keyline.
   ...
   *
   * @return The height of the collapsed bottom sheet in pixels, or {@link #PEEK_HEIGHT_AUTO} if
   * the sheet is configured to peek automatically at 16:9 ratio keyline
   * @attr ref
   * com.google.android.material.R.styleable#BottomSheetBehavior_Layout_behavior_peekHeight
   */
   ...
   * taller as it is increased. The default value is 0.5.
   *
  ...
  public void setHalfExpandedRatio(
    @FloatRange(from = 0.0f, to = 1.0f, fromInclusive = false, toInclusive = false)
      float ratio) {
   ...
   *
   * @return The offset of this bottom sheet within [-1,1] range. Offset increases as this bottom
   * sheet is moving upward. From 0 to 1 the sheet is between collapsed and expanded states
   * and from -1 to 0 it is between hidden and collapsed states. Returns -1 if the bottom
   * sheet is not laid out (therefore it's hidden).
   */
   ...
   * it is expanded once. Setting this to true has no effect unless the sheet is hideable.
   *
   ...
   * @attr ref
   * com.google.android.material.R.styleable#BottomSheetBehavior_Layout_behavior_draggable
   */
   ...
   * @deprecated use {@link #addBottomSheetCallback(MenuSheetCallback)} and {@link
   * #removeBottomSheetCallback(MenuSheetCallback)} instead
   */
  @Deprecated
  public void setBottomSheetCallback(MenuSheetCallback callback) {
    Log.w(
      TAG,
   ...
   * @param callback The callback to notify when bottom sheet events occur.
   */
  public void addBottomSheetCallback(@NonNull MenuSheetCallback callback) {
   ...
   */
  public void removeBottomSheetCallback(@NonNull MenuSheetCallback callback) {
  ...
  private void updateDrawableForTargetState(@State int state, boolean animate) {
    if (state == STATE_SETTLING) {
      // Special case: we want to know which state we're settling to, so wait for another
      // call.
      return;
    }
    ...
    if (animate && interpolatorAnimator != null) {
      ...
      //      }
      //      materialShapeDrawable.setInterpolation(removeCorners ? 0f : 1f);
    } else {
  ...
  private float calculateSlideOffsetWithTop(int top) {
    return (top > collapsedOffset || collapsedOffset == getExpandedOffset())
    ...
    ViewUtils.doOnApplyWindowInsets(
      ...
      new ViewUtils.OnApplyWindowInsetsListener() {
        ...
        public WindowInsetsCompat onApplyWindowInsets(
          ...
          if (paddingBottomSystemWindowInsets) {
            // Intentionally uses getSystemWindowInsetBottom to apply padding
            // properly when
          ...
          view.setPadding(
            leftPadding, view.getPaddingTop(), rightPadding, bottomPadding);

          if (shouldHandleGestureInsets) {
            gestureInsetBottom = mandatoryGestureInsets.bottom;
          }

          // Don't update the peek height to be above the navigation bar or gestures
          // if these
  ...
  /**
   * Checks whether an animation should be smooth after the bottomSheet is released after
   * dragging.
   *
   * @hide
   */
  ...
  public boolean shouldExpandOnUpwardDrag(
    long dragDurationMillis,
    @FloatRange(from = 0.0f, to = 100.0f) float yPositionPercentage) {
    return false;
  }
   ...
   *
   * @return One of {@link #STATE_EXPANDED}, {@link #STATE_HALF_EXPANDED}, {@link
   * #STATE_COLLAPSED}, {@link #STATE_HIDDEN}.
   * @hide
   */
  @State
  @RestrictTo(LIBRARY_GROUP)
  ...
  private void updateImportantForAccessibility(boolean expanded) {
    ...
    if ((VERSION.SDK_INT >= VERSION_CODES.JELLY_BEAN) && expanded) {
      if (importantForAccessibilityMap == null) {
        ...
        // The important for accessibility values of the child views have been saved
        // already.
        return;
      }
    }
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (expanded) {
        ...
        if (updateImportantForAccessibilityOnSiblings
          ...
          ViewCompat.setImportantForAccessibility(
            child, importantForAccessibilityMap.get(child));
        }
      }
    }
    ...
    if (!expanded) {
      ...
      // If the siblings of the bottom sheet have been set to not important for a11y, move the
      // focus
