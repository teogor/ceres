...
import dev.teogor.ceres.ui.spectrum.hct.Cam16.Companion.fromInt
import dev.teogor.ceres.ui.spectrum.util.ColorUtils.argbFromLinrgb
import dev.teogor.ceres.ui.spectrum.util.ColorUtils.argbFromLstar
import dev.teogor.ceres.ui.spectrum.util.ColorUtils.yFromLstar
import dev.teogor.ceres.ui.spectrum.util.MathUtils.matrixMultiply
import dev.teogor.ceres.ui.spectrum.util.MathUtils.sanitizeDegreesDouble
import dev.teogor.ceres.ui.spectrum.util.MathUtils.signum
...
object HctSolver {
  ...
  fun hueOf(linrgb: DoubleArray?): Double {
    val scaledDiscount = matrixMultiply(
      linrgb!!,
   ...
   * cube, in linear RGB coordinates, if it exists. If this possible vertex lies outside of
   * the cube, [-1.0, -1.0, -1.0] is returned.
   */
  ...
  fun solveToInt(hueDegrees: Double, chroma: Double, lstar: Double): Int {
    var hueDegreesCopy = hueDegrees
    if (chroma < 0.0001 || lstar < 0.0001 || lstar > 99.9999) {
      return argbFromLstar(lstar)
    }
    hueDegreesCopy = sanitizeDegreesDouble(hueDegreesCopy)
    val hueRadians = hueDegreesCopy / 180 * Math.PI
  ...
  fun solveToCam(hueDegrees: Double, chroma: Double, lstar: Double): Cam16 {
    return fromInt(solveToInt(hueDegrees, chroma, lstar))
  }
}
